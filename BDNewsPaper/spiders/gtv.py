...
GTV Spider (Bangla)
====================
Scrapes articles from GTV (gabornetv.com)
...
Note:
    As of 2026-03, gabornetv.com is unreachable (ECONNREFUSED).
...
class GTVSpider(BaseNewsSpider):
    """
    Spider for GTV (gabornetv.com).

    Uses RSS feed as primary source, with HTML scraping fallback.

    Usage:
        scrapy crawl gtv
        scrapy crawl gtv -a max_pages=10
    """

    name = 'gtv'
    paper_name = 'GTV'
    allowed_domains = ['gabornetv.com', 'www.gabornetv.com']
    language = 'Bangla'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("GTV spider initialized")
    ...
    def parse_article(self, response: Response) -> Generator[NewsArticleItem, None, None]:
        ...
        body_parts = response.css(
            ...
            '.article-content p::text, .news-content p::text, article p::text'
        ).getall()
        if not body_parts:
