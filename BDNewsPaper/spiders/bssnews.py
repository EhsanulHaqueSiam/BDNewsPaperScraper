"""
BSS News Spider (Bangladesh Sangbad Sangstha)
==============================================
Scrapes articles from BSS News (bssnews.net) - Official Bangladesh News Agency

Features:
    - Sitemap as primary source (1040 URLs)
...
class BSSNewsSpider(BaseNewsSpider):
    """
    Spider for BSS News (Bangladesh Sangbad Sangstha).

    Official national news agency of Bangladesh.
    Uses sitemap as primary source, category scraping as fallback.

    Usage:
        scrapy crawl bssnews -a categories=national,politics,sports
        scrapy crawl bssnews -a max_pages=10
    """

    name = 'bssnews'
    paper_name = 'BSS News'
    allowed_domains = ['bssnews.net', 'www.bssnews.net']
    language = 'English'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(f"BSS News spider initialized")

    def start_requests(self) -> Generator[Request, None, None]:
        """Generate initial requests: sitemap first, then category fallback."""
        self.stats['requests_made'] = 0

        # Primary: Sitemap (1040 URLs)
        self.stats['requests_made'] += 1
        yield Request(
            url='https://bssnews.net/sitemap.xml',
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                url = f"https://www.bssnews.net/{cat_slug}"
            ...
            for cat in default_cats:
                url = f"https://www.bssnews.net/{cat}"
    ...
    def parse_sitemap(self, response: Response) -> Generator:
        """Parse sitemap XML to discover category/section pages for article scraping.

        The BSS sitemap contains category pages (e.g. /news/national) not individual
        article URLs, so we treat them as category pages to crawl for articles.
        """
        ...
        for url_node in urls:
            ...
            # Only follow /news/ category pages (skip /subscriber/ and root pages)
            if '/news/' not in loc:
                continue
            ...
            # Extract category name from URL for metadata
            cat_name = loc.rstrip('/').split('/')[-1]

            yield Request(
                url=loc,
                callback=self.parse_category,
                meta={'category': cat_name, 'cat_slug': cat_name, 'page': 1},
                errback=self.handle_request_failure,
            )

        self.logger.info(f"Sitemap: Queued {sitemap_count} category pages for scraping")

        # If sitemap returned few category pages, also launch default categories
        if sitemap_count < 3:
            self.logger.info("Sitemap returned few pages, launching category fallback")
            yield from self._generate_category_requests()
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Filter to article links with news IDs
        article_links = [l for l in article_links if 'bssnews.net/' in l and '/news/' in l]
        ...
        if found_count > 0 and page < self.max_pages:
            next_page = page + 1
            next_url = f"https://www.bssnews.net/{cat_slug}?page={next_page}"
