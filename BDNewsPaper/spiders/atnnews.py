"""
ATN News Spider (Bangla)
========================
Scrapes articles from atnnewstv.com — Bangladesh's first 24-hour news TV channel.

Note: The original domain atnnews.tv is no longer active. The current domain is
atnnewstv.com. Both are included in allowed_domains for future-proofing.

Features:
    - News sitemap as primary source (news_sitemap.xml with dates, titles, keywords)
...
class ATNNewsSpider(BaseNewsSpider):
    """
    Spider for ATN News (TV News Portal).

    Primary: News sitemap at /news_sitemap.xml (URLs with dates, titles, keywords).
    Fallback: Category HTML pages.

    URL Pattern: atnnewstv.com/details/{id}

    Usage:
        scrapy crawl atnnews
        scrapy crawl atnnews -a categories=politics,country,sports
        scrapy crawl atnnews -a start_date=2026-03-01
    """

    name = 'atnnews'
    paper_name = 'ATN News'
    allowed_domains = ['atnnewstv.com', 'www.atnnewstv.com', 'atnnews.tv', 'www.atnnews.tv']
    language = 'Bangla'
    ...
    # Category slug mappings (atnnewstv.com uses /{slug}/{bangla_name} pattern)
    CATEGORIES = {
        'politics': 'politics',
        'national': 'country',
        'bangladesh': 'country',
    ...
    DEFAULT_CATEGORIES = ['country', 'politics', 'sports', 'economy']

    # Bangla category name mappings for URL construction
    _BANGLA_SLUGS = {
        'politics': 'রাজনীতি',
        'country': 'বাংলাদেশ',
        'world': 'বিশ্ব',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("ATN News spider initialized")
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        sitemap_url = "https://www.atnnewstv.com/news_sitemap.xml"
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        for category in cats:
            ...
            bangla_slug = self._BANGLA_SLUGS.get(cat_slug, '')
            url = f"https://www.atnnewstv.com/{cat_slug}/{bangla_slug}"
    ...
    def parse_sitemap(self, response: Response) -> Generator:
        ...
        for url_node in urls:
            ...
            keywords = url_node.xpath('.//keywords/text()').get('').strip()

            if not loc or 'atnnewstv.com' not in loc:
                continue

            # Only article URLs (details/{id} pattern)
            if '/details/' not in loc:
                continue
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        if found_count > 0 and page < self.max_pages:
            next_page = page + 1
            bangla_slug = self._BANGLA_SLUGS.get(cat_slug, '')
            next_url = f"https://www.atnnewstv.com/{cat_slug}/{bangla_slug}?page={next_page}"
            self.stats['requests_made'] += 1
            yield Request(
                url=next_url,
                callback=self.parse_category,
        ...
        if fallback and fallback.get('headline') and fallback.get('article_body'):
            if len(fallback.get('article_body', '')) >= 100:
                ...
                yield self.create_article_item(
                    ...
                    keywords=response.meta.get('sitemap_keywords'),
                )
                return
