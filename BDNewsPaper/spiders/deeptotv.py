"""
Deepto TV Spider (Bangla)
=========================
Scrapes articles from deeptotv.com — a Bangladeshi TV news channel.

Note: As of March 2026, deeptotv.com has no DNS resolution and appears to be
offline. This spider is pre-built to work once the domain is restored. It
probes for RSS/sitemap on startup and falls back to category HTML scraping.
...
class DeeptoTVSpider(BaseNewsSpider):
    """
    Spider for Deepto TV (TV News Portal).

    Primary: RSS/Sitemap probing (auto-discovers feeds).
    Fallback: Category HTML pages.

    Usage:
        scrapy crawl deeptotv
        scrapy crawl deeptotv -a categories=national,sports,politics
        scrapy crawl deeptotv -a start_date=2026-03-01
    """

    name = 'deeptotv'
    paper_name = 'Deepto TV'
    allowed_domains = ['deeptotv.com', 'www.deeptotv.com']
    language = 'Bangla'
    ...
    # Category slug mappings (typical BD TV news channel pattern)
    CATEGORIES = {
        'national': 'national',
        'bangladesh': 'national',
        'politics': 'politics',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("Deepto TV spider initialized")
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        for endpoint in feed_endpoints:
            url = f"https://www.deeptotv.com{endpoint}"
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        for category in cats:
            ...
            url = f"https://www.deeptotv.com/{cat_slug}"
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        article_links = [
            ...
            if re.search(r'/\d+', l) and (
                'deeptotv.com' in l or l.startswith('/')
            )
        ]
        ...
        if found_count > 0 and page < self.max_pages:
            next_page = page + 1
            next_url = f"https://www.deeptotv.com/{cat_slug}?page={next_page}"
            self.stats['requests_made'] += 1
            yield Request(
                url=next_url,
                callback=self.parse_category,
