"""
Dhaka Courier Spider (English)
==============================
Scrapes articles from Dhaka Courier (dhakacourier.com.bd)
...
class DhakaCourierSpider(BaseNewsSpider):
    """
    Spider for Dhaka Courier (English Weekly Magazine).
    
    Uses HTML scraping with category navigation.
    URL pattern: dhakacourier.com.bd/news/{category}/{slug}/{id}
    
    Usage:
        scrapy crawl dhakacourier -a categories=Politics,Reportage
        scrapy crawl dhakacourier -a max_pages=10
    """
    
    name = 'dhakacourier'
    paper_name = 'Dhaka Courier'
    allowed_domains = ['dhakacourier.com.bd']
    language = 'English'
    ...
    CATEGORIES = {
        ...
        'column': 'Column',
        'essays': 'Essays',
        'world': 'World_this_week',
        'nation': 'Nation_this_week',
        'society': 'Society',
        'global': 'Global',
        'interview': 'Interview',
    }
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                cat_slug = self.CATEGORIES.get(cat_lower, category)
                url = f"https://dhakacourier.com.bd/news/{cat_slug}"
            ...
            # Default categories
            for cat in ['Politics', 'Reportage', 'Column', 'Featured_1']:
                url = f"https://dhakacourier.com.bd/news/{cat}"
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Extract article links - Dhaka Courier uses /news/Category/slug/id pattern
        article_links = response.css('a::attr(href)').getall()
        article_links = [
            l for l in article_links 
            if 'dhakacourier.com.bd/news/' in l 
        ...
        for url in article_links:
            if not url.startswith('http'):
                url = f"https://dhakacourier.com.bd{url}"
        ...
        if found_count > 0 and page < self.max_pages:
            ...
            if next_page_link:
                if not next_page_link.startswith('http'):
                    next_page_link = f"https://dhakacourier.com.bd{next_page_link}"
                
                self.stats['requests_made'] += 1
                yield Request(
                    url=next_page_link,
                    callback=self.parse_category,
                    meta={'category': category, 'cat_slug': response.meta.get('cat_slug'), 'page': page + 1},
                    errback=self.handle_request_failure,
                )
