...
Features:
    - Google News sitemap as primary source (fresh)
...
class ITVBDSpider(BaseNewsSpider):
    ...
    Uses Google News sitemap as primary source, with HTML scraping fallback.
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        # Primary: Google News sitemap (fresh)
        self.stats['requests_made'] += 1
        yield Request(
            url='https://itvbd.com/news-sitemap.xml',
            callback=self.parse_news_sitemap,
    ...
    def _sitemap_failed(self, failure):
        ...
        self.logger.warning(f"News sitemap failed: {failure.value}. Falling back to category scraping.")
        self.stats['errors'] += 1
    ...
    # ================================================================
    # News Sitemap Parsing (Primary Source)
    # ================================================================

    def parse_news_sitemap(self, response: Response) -> Generator:
        """Parse Google News sitemap XML to extract article URLs.

        Google News sitemaps use <url> with <loc>, <lastmod>, and
        <news:news> containing <news:publication_date> and <news:title>.
        """
        ...
        for url_node in urls:
            ...
            yield Request(
                ...
                meta={'category': 'General', 'source': 'sitemap', 'sitemap_title': title},
                errback=self.handle_request_failure,
            )

        self.logger.info(f"News sitemap: Yielded {sitemap_count} article requests")
