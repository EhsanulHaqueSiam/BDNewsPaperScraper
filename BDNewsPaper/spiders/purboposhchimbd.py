"""
Purboposhchim BD Spider (Bangla)
=================================
Scrapes articles from Purboposhchim BD (purboposhchimbd.news)

Features:
    - RSS feed as primary source (if available)
    - HTML scraping with universal fallback
    - Date filtering (client-side)
    - Search query filtering

Note:
    As of 2026-03, purboposhchimbd.news appears to be a parked domain.
    This spider is prepared for when the site becomes active again.
    The domain returned 400 with anti-bot blocking on direct requests.
"""
...
class PurboposhchimBDSpider(BaseNewsSpider):
    """
    Spider for Purboposhchim BD (purboposhchimbd.news).

    Uses RSS feed as primary source, with HTML scraping fallback.

    Usage:
        scrapy crawl purboposhchimbd
        scrapy crawl purboposhchimbd -a max_pages=10
    """

    name = 'purboposhchimbd'
    paper_name = 'Purboposhchim BD'
    allowed_domains = ['purboposhchimbd.news', 'www.purboposhchimbd.news']
    language = 'Bangla'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("Purboposhchim BD spider initialized")
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        yield Request(
            url='https://purboposhchimbd.news/feed',
    ...
    def _rss_failed(self, failure):
        ...
        self.stats['requests_made'] += 1
        yield Request(
            url='https://purboposhchimbd.news/',
    ...
    def parse_article(self, response: Response) -> Generator[NewsArticleItem, None, None]:
        ...
        body_parts = response.css(
            ...
            '.article-body p::text, .news-content p::text, article p::text'
        ).getall()
        if not body_parts:
