"""
Bangladesh Pratidin Spider (Bangla)
===================================
Scrapes articles from Bangladesh Pratidin (bd-pratidin.com) - Highest circulation Bangla

Features:
    - RSS feed as primary source (348 items)
...
class BDPratidinBanglaSpider(BaseNewsSpider):
    """
    Spider for Bangladesh Pratidin Bangla edition.

    Uses RSS feed as primary source, HTML scraping as fallback.

    Usage:
        scrapy crawl bdpratidin_bangla -a categories=national,politics,sports
        scrapy crawl bdpratidin_bangla -a max_pages=10
    """

    name = 'bdpratidin_bangla'
    paper_name = 'Bangladesh Pratidin Bangla'
    allowed_domains = ['bd-pratidin.com', 'www.bd-pratidin.com']
    language = 'Bangla'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(f"BD Pratidin Bangla spider initialized")

    def start_requests(self) -> Generator[Request, None, None]:
        """Generate initial requests: RSS feed first, then category fallback."""
        self.stats['requests_made'] = 0

        # Primary: RSS feed (348 items)
        self.stats['requests_made'] += 1
        yield Request(
            url='https://bd-pratidin.com/rss.xml',
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                url = f"https://www.bd-pratidin.com/{cat_slug}"
            ...
            default_cats = ['first-page', 'politics', 'sport', 'economics']
            for cat in default_cats:
                url = f"https://www.bd-pratidin.com/{cat}"
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Filter to article links on bd-pratidin.com - actual pattern: /category-slug/YYYY/MM/DD/ID
        article_links = [
            l for l in article_links
            if 'bd-pratidin.com/' in l
            and re.search(r'/[^/]+/\d{4}/\d{2}/\d{2}/\d+', l)
        ]
        ...
        if found_count > 0 and page < self.max_pages:
            next_page = page + 1
            next_url = f"https://www.bd-pratidin.com/{cat_slug}?page={next_page}"
