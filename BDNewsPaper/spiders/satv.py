...
SA TV Spider (Bangla)
======================
Scrapes articles from SA TV (satv.tv)
...
class SATVSpider(BaseNewsSpider):
    """
    Spider for SA TV (satv.tv).
    ...
    Usage:
        scrapy crawl satv
        scrapy crawl satv -a categories=bangladesh,sports
        scrapy crawl satv -a max_pages=10
    """

    name = 'satv'
    paper_name = 'SA TV'
    allowed_domains = ['satv.tv', 'www.satv.tv']
    language = 'Bangla'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("SA TV spider initialized")
    ...
    def parse_homepage(self, response: Response) -> Generator:
        ...
        # Filter to article links on satv.tv domain
        filtered_links = []
        for link in article_links:
            full_url = response.urljoin(link)
            # SATV uses Bengali-encoded URL slugs for articles
            if 'satv.tv/' in full_url and full_url.rstrip('/') != 'https://www.satv.tv':
                # Skip category/tag pages and non-article paths
                skip_patterns = ['/category/', '/tag/', '/page/', '/author/', '/feed', '/wp-']
                if not any(p in full_url for p in skip_patterns):
    ...
    def parse_article(self, response: Response) -> Generator[NewsArticleItem, None, None]:
        ...
        # Manual extraction fallback (SATV-specific selectors)
        headline = (
            response.css('.single-page-title::text').get() or
        ...
        body_parts = response.css(
            '.editorial_content p::text, .entry-content p::text, '
            '.post-content p::text, article p::text'
        ).getall()
        if not body_parts:
        ...
        # Remove WordPress "The post ... appeared first on" suffix
        article_body = re.sub(
            r'\s*The post .+ appeared first on .+\.\s*$', '', article_body
        ).strip()
        ...
        date_text = (
            ...
            response.css('.header-date23::text').get() or
            ''
        )
        if date_text:
