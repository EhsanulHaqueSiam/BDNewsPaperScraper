"""
Bonik Barta Spider (Bangla)
===========================
Scrapes articles from Bonik Barta (bonikbarta.com) - Leading Bangla Business Daily

Features:
    - Daily sitemap as primary source (27 articles/day)
    - SPA rendering via Scrapling stealthy mode
    - Category-based scraping as fallback
    - Date filtering (client-side)
    - Business/economy news focus
"""
...
class BonikBartaSpider(BaseNewsSpider):
    """
    Spider for Bonik Barta (Bangla Business Newspaper).

    Uses daily sitemap as primary source with Scrapling stealthy mode for SPA rendering.

    Usage:
        scrapy crawl bonikbarta -a categories=economy,business,national
        scrapy crawl bonikbarta -a max_pages=10
    """

    name = 'bonikbarta'
    paper_name = 'Bonik Barta'
    allowed_domains = ['bonikbarta.com', 'www.bonikbarta.com']
    language = 'Bangla'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(f"Bonik Barta spider initialized")

    def start_requests(self) -> Generator[Request, None, None]:
        """Generate initial requests: today's sitemap first, then category fallback."""
        self.stats['requests_made'] = 0

        # Primary: Today's daily sitemap (27 articles/day)
        today = datetime.now().strftime('%Y-%m-%d')
        sitemap_url = f"https://bonikbarta.com/sitemaps/sitemap-news-{today}.xml?path=00000000010000000001"
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                url = f"https://www.bonikbarta.com/{cat_slug}"
            ...
            default_cats = ['economy', 'business', 'stock-market', 'banking']
            for cat in default_cats:
                url = f"https://www.bonikbarta.com/{cat}"
                ...
                yield Request(
                    url=url,
                    callback=self.parse_category,
                    meta={'category': cat, 'cat_slug': cat, 'page': 1, 'scrapling': 'stealthy'},
                    errback=self.handle_request_failure,
                )
    ...
    def parse_sitemap(self, response: Response) -> Generator:
        ...
        for url_node in urls:
            ...
            # Only follow bonikbarta.com URLs
            if 'bonikbarta.com/' not in loc:
                continue
            ...
            # SPA needs browser rendering via Scrapling
            yield Request(
                url=loc,
                callback=self.parse_article,
                meta={'category': 'Business', 'scrapling': 'stealthy'},
                errback=self.handle_request_failure,
            )

        self.logger.info(f"Sitemap: Queued {sitemap_count} articles for scraping")

        # If sitemap returned few items, also launch category fallback
        if sitemap_count < 5:
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Filter to article links (contain bonikbarta.com and numeric IDs)
        article_links = [l for l in article_links if 'bonikbarta.com/' in l and re.search(r'/\d+$', l)]
        ...
        if found_count > 0 and page < self.max_pages:
            next_page = page + 1
            next_url = f"https://www.bonikbarta.com/{cat_slug}?page={next_page}"
