"""
Channel I Online Spider (Bangla)
================================
Scrapes articles from Channel I Online (channelionline.com)
...
class ChannelISpider(BaseNewsSpider):
    """
    Spider for Channel I Online (TV News Portal).

    Uses RSS feed as primary source, with HTML scraping fallback.

    Usage:
        scrapy crawl channeli -a categories=bangladesh,politics
        scrapy crawl channeli -a max_pages=10
    """

    name = 'channeli'
    paper_name = 'Channel I Online'
    allowed_domains = ['channelionline.com', 'www.channelionline.com']
    language = 'Bangla'
    ...
    CATEGORIES = {
        'bangladesh': 'category/bangladesh',
        'national': 'category/bangladesh',
        'politics': 'category/politics',
        'international': 'category/international',
        'world': 'category/international',
        'sports': 'sports',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(f"Channel I spider initialized")
    ...
    def start_requests(self) -> Generator[Request, None, None]:
        ...
        yield Request(
            url='https://channelionline.com/feed',
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                cat_slug = self.CATEGORIES.get(cat_lower, f"category/{cat_lower}")
                url = f"https://www.channelionline.com/{cat_slug}/"
            ...
            for cat in ['bangladesh', 'politics', 'sports', 'international']:
                cat_slug = self.CATEGORIES.get(cat, f"category/{cat}")
                url = f"https://www.channelionline.com/{cat_slug}/"
                self.stats['requests_made'] += 1
                yield Request(
                    url=url,
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Filter to article links (Channel I uses /slug-name/ pattern)
        article_links = [l for l in article_links if 'channelionline.com' in l and re.search(r'/[a-z0-9-]+/$', l) and '/category/' not in l]
        ...
        if found_count > 0 and page < self.max_pages:
            next_url = f"https://www.channelionline.com/{cat_slug}/page/{page + 1}/"
            self.stats['requests_made'] += 1
            yield Request(
                url=next_url,
                callback=self.parse_category,
    ...
    def parse_article(self, response: Response) -> Generator[NewsArticleItem, None, None]:
        ...
        body_parts = response.css('article p::text, .entry-content p::text').getall()
        if not body_parts:
