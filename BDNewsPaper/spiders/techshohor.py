"""
Tech Shohor Spider (Bangla)
===========================
Scrapes articles from Tech Shohor (techshohor.com)

Features:
    - RSS feed as primary source (~11 items, WordPress)
...
class TechShohorSpider(BaseNewsSpider):
    """
    Spider for Tech Shohor (Bangla Tech News).

    Uses RSS feed as primary source, with HTML scraping fallback.
    Covers technology, mobile, software, and startup news.

    Usage:
        scrapy crawl techshohor -a categories=handset,software
        scrapy crawl techshohor -a max_pages=10
    """

    name = 'techshohor'
    paper_name = 'Tech Shohor'
    allowed_domains = ['techshohor.com', 'www.techshohor.com']
    language = 'Bangla'
    ...
    # Category slug mappings (URL encoded Bangla)
    CATEGORIES = {
        'recent': '%e0%a6%b8%e0%a6%be%e0%a6%ae%e0%a7%8d%e0%a6%aa%e0%a7%8d%e0%a6%b0%e0%a6%a4%e0%a6%bf%e0%a6%95',
        'handset': 'product-reviews/handset',
        'mobile': 'product-reviews/handset',
        'computer': 'product-reviews/computer',
        'software': 'software',
        'tutorial': 'tutorial',
        'tips': 'tutorial',
        'business': 'business',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("Tech Shohor spider initialized (Bangla Tech)")

    def start_requests(self) -> Generator[Request, None, None]:
        """Generate initial requests: RSS feed first, then category fallback."""
        self.stats['requests_made'] = 0

        # Primary: RSS feed (WordPress, ~11 items, fresh)
        self.stats['requests_made'] += 1
        yield Request(
            url='https://techshohor.com/feed',
    ...
    def _generate_category_requests(self) -> Generator[Request, None, None]:
        ...
        if self.categories:
            for category in self.categories:
                ...
                url = f"https://techshohor.com/{cat_slug}/"
    ...
    def parse_rss(self, response: Response) -> Generator:
        ...
        for item in items:
            ...
            category = item.xpath('category/text()').get('Technology').strip()
        ...
        # RSS has only ~11 items, supplement with category scraping
        yield from self._generate_category_requests()
    ...
    def parse_category(self, response: Response) -> Generator:
        ...
        # Extract article links - Pattern: /ID/slug/
        article_links = response.css('a::attr(href)').getall()
        article_links = [
            l for l in article_links
            if 'techshohor.com/' in l
            and re.search(r'/\d{5,}/', l)  # Article IDs are 5+ digits
        ...
        for url in article_links:
            if not url.startswith('http'):
                url = f"https://techshohor.com{url}"
        ...
        if found_count > 0 and page < self.max_pages:
            ...
            if next_page_link:
                if not next_page_link.startswith('http'):
                    next_page_link = f"https://techshohor.com{next_page_link}"
    ...
    def parse_article(self, response: Response) -> Generator[NewsArticleItem, None, None]:
        ...
        category = response.meta.get('category', 'Technology')
